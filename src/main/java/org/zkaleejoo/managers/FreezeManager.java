...
public class FreezeManager {
    ...
    private void spawnFrozenDisplay(Player target) {
        ...
        FoliaCompat.runAsync(plugin, () -> {
            ...
            FoliaCompat.runForEntity(plugin, target, () -> {
                ...
                try {
                    ...
                    TextDisplay textDisplay = target.getWorld().spawn(calculateDisplayLocation(target),
                            ...
                            display -> {
                                ...
                                display.getPersistentDataContainer().set(freezeDisplayFlagKey, PersistentDataType.BYTE,
                                        (byte) 1);
                                display.getPersistentDataContainer().set(freezeDisplayOwnerKey,
                    ...
                    WrappedTask trackingTask = FoliaCompat.runForEntityTimer(plugin, target, () -> {
                        ...
                        FoliaCompat.teleport(textDisplay, calculateDisplayLocation(target));
                    }, 0L, updateTicks);
    ...
    public void cleanupOrphanDisplays() {
        for (World world : Bukkit.getWorlds()) {
            for (Entity entity : world.getEntities()) {
                ...
                String ownerRaw = display.getPersistentDataContainer().get(
                        Objects.requireNonNull(freezeDisplayOwnerKey),
    ...
    private void removeOrphanDisplaysFor(UUID ownerUuid) {
        ...
        for (World world : Bukkit.getWorlds()) {
            for (Entity entity : world.getEntities()) {
                ...
                String displayOwner = display.getPersistentDataContainer().get(
                        Objects.requireNonNull(freezeDisplayOwnerKey),
                        Objects.requireNonNull(PersistentDataType.STRING));
    ...
    private boolean isFreezeDisplayEntity(TextDisplay display) {
        Byte marker = display.getPersistentDataContainer().get(Objects.requireNonNull(freezeDisplayFlagKey),
                Objects.requireNonNull(PersistentDataType.BYTE));
    ...
    private static class FrozenDisplayTracker {
        ...
        private FrozenDisplayTracker(TextDisplay display, WrappedTask task, String lastText, long refreshEveryRuns) {
