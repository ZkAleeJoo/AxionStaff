...
public class DiscordManager {
    ...
    private void sendWebhook(String type, Map<String, String> placeholders, String nameForImage) {
        ...
        FoliaCompat.runAsync(plugin, () -> {
            try {
                ...
                String webhookUrl = resolveWebhookUrl(section);
                ...
                String footerText = resolveText(section, "footer",
                        discordConfig.getConfig().getString("footer", defaultFooter), safePlaceholders,
                        path + ".footer");
    ...
    private String resolveWebhookUrl(ConfigurationSection section) {
        String globalWebhookUrl = sanitizeOrDefault(discordConfig.getConfig().getString("webhook-url"), "");
        String sectionWebhookUrl = sanitizeOrDefault(section.getString("webhook-url"), "");

        if (discordConfig.getConfig().getBoolean("use-global-webhook", true)) {
            return isConfiguredWebhook(globalWebhookUrl) ? globalWebhookUrl : "";
        }

        if (isConfiguredWebhook(sectionWebhookUrl)) {
            return sectionWebhookUrl;
        }

        return isConfiguredWebhook(globalWebhookUrl) ? globalWebhookUrl : "";
    }

    private boolean isConfiguredWebhook(String webhookUrl) {
        return webhookUrl != null && !webhookUrl.isBlank() && !webhookUrl.contains("URL_AQUI");
    }

    private String resolveText(ConfigurationSection section, String key, String defaultValue,
        ...
        if (resolvedValue.length() > 4096) {
            plugin.getLogger()
                    .warning("Discord text in " + configPath + " is too long. Truncating to 4096 characters.");
            return resolvedValue.substring(0, 4096);
        }
        return resolvedValue;
    }

    private String resolveImage(ConfigurationSection section, String key, String defaultValue,
            Map<String, String> placeholders,
            String faceUrl) {
        String rawValue = sanitizeOrDefault(section.getString(key), defaultValue);
