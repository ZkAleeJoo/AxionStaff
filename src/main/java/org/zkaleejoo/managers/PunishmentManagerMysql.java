...
public class PunishmentManagerMysql extends AbstractPunishmentManager {
    ...
    private void startMuteCacheCleanupTask() {
        muteCacheCleanupTask = FoliaCompat.runAsyncTimer(plugin, this::cleanupMuteCaches,
    ...
    protected void executeAfterHistoryLog(Runnable action) {
        FoliaCompat.runGlobalLater(plugin, action, 5L);
    }
    ...
    public void cleanupExpiredPunishments(CommandSender requester) {
        ...
        FoliaCompat.runAsync(plugin, () -> {
            ...
            try (Connection conn = db.getConnection()) {
                try (PreparedStatement ps = conn
                        .prepareStatement("DELETE FROM MaxStaff_bans WHERE expiry <> -1 AND expiry <= ?")) {
                    ps.setLong(1, now);
                    bansRemoved = ps.executeUpdate();
                }

                try (PreparedStatement ps = conn
                        .prepareStatement("DELETE FROM MaxStaff_mutes WHERE expiry <> -1 AND expiry <= ?")) {
                    ps.setLong(1, now);
                    mutesRemoved = ps.executeUpdate();
                }

                try (PreparedStatement ps = conn
                        .prepareStatement("DELETE FROM MaxStaff_ip_bans WHERE expiry <> -1 AND expiry <= ?")) {
                    ps.setLong(1, now);
        ...
        FoliaCompat.runAsync(plugin, () -> {
            try (Connection conn = db.getConnection()) {
                if (type.equalsIgnoreCase("all")) {
                    try (PreparedStatement ps = conn
                            .prepareStatement("DELETE FROM MaxStaff_history WHERE uuid = ?")) {
                        ps.setString(1, uuid.toString());
                        ps.executeUpdate();
                    }
                } else {
    ...
    private void banIPPlayerInternal(CommandSender staff, String target, String reason, String durationStr,
        ...
        if (!IPUtils.isValidIp(ip)) {
            plugin.getLogger().warning("/ban-ip resolved a non-IP value for target " + target + ": " + ip);
            if (!silent) {
                staff.sendMessage(
                        prefixed("&cThe resolved IP is not valid: &f" + ip + " &cUse literal IP."));
            }
            return;
        }
    ...
    private String resolveUnbanIpTarget(String target) {
        ...
        String fallbackIp = normalizeIp(getIpFromIpBansAssociation(target));
        if (fallbackIp != null && !fallbackIp.isEmpty() && IPUtils.isValidIp(fallbackIp)) {
            return fallbackIp;
        }
    ...
    public String getActiveIPBanMessage(String ip) {
        try (Connection conn = db.getConnection();
                PreparedStatement ps = conn
                        .prepareStatement(
                                "SELECT reason, staff, expiry FROM MaxStaff_ip_bans WHERE ip = ? LIMIT 1")) {
            ps.setString(1, ip);
            try (ResultSet rs = ps.executeQuery()) {
                if (!rs.next()) {
                    return null;
    ...
    public String getPlayerName(UUID uuid) {
        String name = resolveLastKnownName(uuid);
        if (name == null || name.isBlank()) {
            String offlineName = Bukkit.getOfflinePlayer(uuid).getName();
            name = offlineName == null || offlineName.isBlank() ? uuid.toString() : offlineName;
        }
        return name;
    }
