...
public class GuiManager {
    ...
    public void openUserInfoMenu(Player staff, Player target) {
        FoliaCompat.runAsync(plugin, () -> {
            ...
            FoliaCompat.runForEntity(plugin, staff, () -> {
                if (!staff.isOnline())
                    return;
    ...
    public void openActivePunishmentsMenu(Player staff, int page) {
        ...
        if (plugin.getMainConfigManager().isDbEnabled()) {
            FoliaCompat.runAsync(plugin, loadRecords);
        } else {
            loadRecords.run();
        }
    }
    ...
    private void startActivePunishmentsRefresh(Player staff) {
        ...
        FoliaCompat.WrappedTask task = FoliaCompat.runForEntityTimer(plugin, staff, () -> {
    ...
    private void cancelActivePunishmentsRefresh(UUID staffId) {
        FoliaCompat.WrappedTask existing = activePunishmentRefreshTasks.remove(staffId);
        if (existing != null) {
            existing.cancel();
        }
    }
