...
import org.zkaleejoo.utils.InvSeeSyncTask;
...
public class GuiListener implements Listener {
    ...
    public void onInventoryClose(InventoryCloseEvent event) {
        ...
        if (isEditableOnlineInspection(holder, player)) {
            InvSeeSyncTask session = activeSyncSessions.remove(player.getUniqueId());
    ...
    public void startSyncSession(Player staff, Player target, Inventory gui) {
        endSyncSession(staff);
        MainConfigManager config = plugin.getMainConfigManager();
        InvSeeSyncTask task = new InvSeeSyncTask(plugin, staff, target, gui, config);
        activeSyncSessions.put(staff.getUniqueId(), task);
        task.start();
    }

    public void endSyncSession(Player staff) {
        InvSeeSyncTask task = activeSyncSessions.remove(staff.getUniqueId());
        if (task != null) {
            task.stop();
        }
    }

    public void endAllSyncSessions() {
        for (InvSeeSyncTask task : activeSyncSessions.values()) {
            task.stop();
        }
    ...
    private ItemStack cloneInventoryItem(ItemStack item) {
        if (item == null || item.getType() == Material.AIR
                || InspectionInventoryBuilder.isInspectionPlaceholder(item)) {
            return null;
        }
        return item.clone();
    }
        ...
        if (mat == config.getGuiInfoActionMat()) {
            ...
            if (target != null) {
                ...
                Inventory inspection = Objects.requireNonNull(InspectionInventoryBuilder.createOnlineInspection(
                        ...
                        config,
                        editable));
                player.openInventory(inspection);
                if (editable) {
                    startSyncSession(player, target, inspection);
                }
                return;
            } else {
        ...
        if ("tp_player".equals(action) && item.getType() == Material.PLAYER_HEAD) {
            ...
            if (target == null) {
                String cleanName = meta.hasDisplayName()
                        ? PlainTextComponentSerializer.plainText().serialize(Objects.requireNonNull(meta.displayName()))
                        : "";
    ...
    private void handlePermissionsMenu(Player player, ItemStack item, MaxStaffHolder holder,
    ...
    private void handleReviveMenu(Player player, ItemStack item, MainConfigManager config) {
        ...
        if ((targetName == null || targetName.isBlank()) && meta.hasDisplayName()) {
            targetName = PlainTextComponentSerializer.plainText().serialize(Objects.requireNonNull(meta.displayName()));
        }
