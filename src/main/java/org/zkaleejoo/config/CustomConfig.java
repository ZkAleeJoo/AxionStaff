...
public class CustomConfig {
    ...
    public void registerConfig() {
        ...
        if (!file.exists()) {
            if (newFile) {
                ...
                if (folderName != null) {
                    plugin.saveResource(Objects.requireNonNull(folderName + File.separator + fileName), false);
                } else {
        ...
        try {
            ...
            for (String key : jarConfig.getKeys(true)) {
                if (!fileConfiguration.contains(key)) {
                    if (shouldSkipMissingBundledKey(key, fileConfiguration)) {
                        continue;
                    } else {
    ...
    static boolean shouldSkipMissingBundledKey(String key, FileConfiguration fileConfiguration) {
        ...
        return fileConfiguration.contains(getProtectedParent(key));
    }

    private static boolean isInsideProtectedSection(String key) {
        return key.startsWith("punishment-reasons.")
                || key.startsWith("gui-gamemode.")
