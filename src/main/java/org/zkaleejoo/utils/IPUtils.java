...
public final class IPUtils {
    ...
    private static final Pattern IPV6_LITERAL_PATTERN = Pattern.compile("^([0-9a-fA-F]{0,4}:){2,7}[0-9a-fA-F]{0,4}$");
    ...
    /**
     * Returns {@code true} if the given string represents a valid IPv4 or IPv6
     * address after normalisation (leading {@code /} stripped, trimmed).
     * Unlike {@link #isLiteralIp(String)}, this method accepts the raw forms
     * returned by {@link java.net.InetAddress#toString()} (e.g. {@code /127.0.0.1}).
     */
    public static boolean isValidIp(String value) {
        String normalized = normalizeIp(value);
        return normalized != null && isLiteralIp(normalized);
    }
}
